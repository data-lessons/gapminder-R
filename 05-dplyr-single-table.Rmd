...
Okay, now we want to see just the rows of the data.frame where country is "United States". The synatx for all `dplyr` functions is the same: The first argument is the data.frame, the rest of the arguments are whatever you want to do in that data.frame. 
...
We can also use "or" conditions with the verticle pipe: `|`. Notice that the variable (column) names don't go in quotes, but values of character variables do. 
...
You can order the rows of a data.frame by a variable using `arrange`. Suppose we want to see the most populus countries. Again, we wrap the results in `head` to just print the first few rows: 

```{r arrange}
head(arrange(gapminder, pop))
```

Hmm, we didn't get the most populus countries. By default, `arrange` sorts the variable in *increasing* order. We could see the most populus countries by examining the `tail` of the last command, or we can sort the data.frame by descending population by wrapping the variable in `desc()`:
...
Suppose we want to look at all the countries where life expectency is greater than 80 years, sorted from poorest to richest. First, we `filter`, then we `arrange`. Each function expects a data.frame as its first argument and returns a data.frame as its output. So we could wrap them like we did with `head(select( ... ))` above, or we could assign the intermediate data.frame to a varaible.

```{r filter and arrange}
arrange(filter(gapminder, lifeExp > 80), gdpPercap)
```

That line of code is pretty difficult to read. There is a better way, which makes both writing and reading the code easier. The pipe from the `magrittr` package (which is automatically installed and loaded with `dplyr`) takes whatever is on its left and makes it sends it to whatever is on its right. 

```{r pipe simple}
4 %>% sqrt()
```

More specifically, whatever goes through the pipe becomes the first argument of the function after the pipe. This is convenient, because all `dplyr` functions produce a data.frame as their output and take a data.frame as the first argument. Since R ignores white-space, we can put each function on a new line, which RStudio will automatically indent, making everything easy to read. Now each line represents a step in a sequential operation. You can read this as "Take the gapminder data.frame, filter to the rows where lifeExp is greater than 80, and arrange by gdpPercap." 
...
You can create multiple columns in the same function call, separating them by a comma. E.g. suppose we want the base-10 logarithm of each country's population and per-capita income:
...
Note that `summarize` eleminates any other columns. Why? What else can it do? E.g. What country should it list for the year 1952!?

You can also do multiple groupings. Suppose we want the average life expectency by continent and year. 
...
> - Calculate the variance -- `var()` of countries' incomes at each year.
...
That is the core of `dplyr`'s functionality, but it does more. RStudio makes a great [cheatsheet](https://www.rstudio.com/wp-content/uploads/2015/02/data-wrangling-cheatsheet.pdf) that covers all the `dplyr` functions we just learned, plus what we will learn in the next two lessons: keeping data tidy and merging multiple data.frames.