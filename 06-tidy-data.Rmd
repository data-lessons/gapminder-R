...
Data can be organized many ways. While there may be times that call for other organizational schemes, in general it is best to organize data thus:

1. Each variable forms a column
1. Each observation forms a row
1. Each observational type forms a table

What exactly constitutes a variable can be difficult to define out of context, but as a general rule, if observations are measured in the same units, they should be in different columns, and if they are measured in the same units, there is a good chance they should be in the same column.
...
We can transform the data to that form with the `gather` function from the `tidyr` package. Of course, if you haven't yet installed `tidyr`, you'll need to do that using `install.packages(tidyr)`, and we need to load it.
...
You can also tell `gather` which columns not to gather -- these are the "ID variables".
...
> ```
>
> You could argue that it is or is not in tidy form, but go ahead and `gather` it so that each measurement of RSD is on its own row.
> 
> Then, make a line plot with two lines, one for each measure of relative variance in income, by year. To make the plot black-and-white-printer friendly, distinguish the lines using the `linetype` **aes**thetic.
>

### Joins

The third tidy data maxim states that each observation type gets its own table. This will be familiar to anyone who has worked with a relational database but may seem foreign to those who have not. 

The idea is this: Suppose we conduct a behavioral experiment that puts individuals in groups and measure individual- and group-level variables. We should have a table for the individual-level variables and a separate table for the group-level variables. Then, should we need to merge them, we can do so using the `join` functions of `dplyr`. 
...
We will practice on our continents data.frame from module 2 and the gapminder data.frame. Note that these are tidy data: We have observations at the level of continent and at the level of country, so they go in different tables. The continent column in the gapminder data.frame allows us to link them now. If it isn't in your Environment, load it and remember what it consists of:
...
We can join the two data.frames using any of the `dplyr` functions. 
...
> library(ggplot2)
> 
> mutate(gapminder, GDP = gdpPercap * pop) %>%  # Calculate country-level GDP
>     group_by(continent) %>%  # Group by continent
>     summarize(cont_income = sum(GDP)) %>%  # Calculate continent-level GDP
>     left_join(continents) %>%  # Join the continent-GDP data.frame to the continents data.frame
>     mutate(per_cap = cont_income / population) %>%  # Calculate continent-level per-capita GDP
>     ggplot(aes(x = area_km2, y = per_cap)) +  # Plot income versus land area
>     geom_point() +  # Draw points
>     geom_text(aes(label = continent), nudge_y = 5e3)  # And label them
> ```
>
